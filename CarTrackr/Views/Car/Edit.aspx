...
<asp:Content ID="editContent" ContentPlaceHolderID="MainContent" runat="server">
    <h2>Edit <%=Html.Encode(Model.Make)%> <%=Html.Encode(Model.Model)%> - <%=Html.Encode(Model.LicensePlate)%></h2>
    ...
    <% using (Html.BeginForm("Edit", "Car", FormMethod.Post)) { %>
        ...
        <table border="0" cellpadding="2" cellspacing="0" class="vertical">
            ...
            <tr>
                <th>License plate</th>
                <td>
                    <%=Html.TextBox("NewLicensePlate", Model.LicensePlate ?? "")%>
        ...
        <%=Html.ActionLink("Cancel", "Details", "Car", new { licensePlate = Model.LicensePlate }, null)%>
    <% } %>
</asp:Content>
